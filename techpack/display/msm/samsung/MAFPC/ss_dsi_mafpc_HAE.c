...
#include "ss_dsi_mafpc_HAE.h"

/*
 * make dsi_panel_cmds using image data
 */
int ss_mafpc_make_img_cmds_HAE(struct samsung_display_driver_data *vdd, char *data,
...
int ss_mafpc_make_img_mass_cmds_HAE(struct samsung_display_driver_data *vdd, char* data,
									u32 data_size, int cmd_type)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
...
#define BUF_LEN 200
int ss_mafpc_update_enable_cmds_HAE(struct samsung_display_driver_data *vdd)
{
	struct dsi_panel_cmd_set *pcmds;
...
struct dsi_panel_cmd_set *ss_mafpc_brightness_scale_HAE(struct samsung_display_driver_data *vdd, int *level_key)
{
			 ...
			 size_t total_count, loff_t *ppos)
{
	...
	ret = ss_mafpc_update_enable_cmds_HAE(vdd);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_update_enable_cmds_HAE\n");
		return ret;
	}
	ret = ss_mafpc_make_img_mass_cmds_HAE(vdd, vdd->mafpc.img_buf, vdd->mafpc.img_size, TX_MAFPC_IMAGE);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_make_img_mass_cmds_HAE\n");
		return ret;
	}
...
*/
static int update_mafpc_scale_HAE(struct samsung_display_driver_data *vdd,
...
int ss_mafpc_init_HAE(struct samsung_display_driver_data *vdd)
{
	int ret = 0;
	...
	vdd->mafpc.make_img_mass_cmds = ss_mafpc_make_img_mass_cmds_HAE;
	vdd->mafpc.make_img_cmds = ss_mafpc_make_img_cmds_HAE;
	vdd->mafpc.img_write = ss_mafpc_img_write;
	vdd->mafpc.debug = ss_mafpc_debug;
	vdd->panel_func.br_func[BR_FUNC_MAFPC_SCALE] = ss_mafpc_brightness_scale_HAE;

	vdd->mafpc.brightness_scale_table_size = sizeof(brightness_scale_table);

	vdd->mafpc.enable_cmd_size = MAFPC_ENABLE_COMMAND_LEN_HAE;
	vdd->mafpc.enable_cmd_buf = kzalloc(MAFPC_ENABLE_COMMAND_LEN_HAE, GFP_KERNEL);
	...
	register_op_sym_cb(vdd, "MAFPC", update_mafpc_scale_HAE, true);
