...
#include "ss_dsi_mafpc_XA2.h"

/*
 * make dsi_panel_cmds using image data
 */
int ss_mafpc_make_img_cmds_XA2(struct samsung_display_driver_data *vdd, char *data,
...
int ss_mafpc_make_img_mass_cmds_XA2(struct samsung_display_driver_data *vdd, char* data,
									u32 data_size, int cmd_type)
{
	struct dsi_cmd_desc *tcmds;
	struct dsi_panel_cmd_set *pcmds;
...
#define BUF_LEN 200
int ss_mafpc_update_enable_cmds_XA2(struct samsung_display_driver_data *vdd)
{
	struct dsi_panel_cmd_set *pcmds;
...
static int ss_mafpc_img_write(struct samsung_display_driver_data *vdd, bool is_instant)
{
	...
	if (vdd->is_factory_mode || is_instant || vdd->mafpc.force_delay) {
		/*
		 * +1 means padding.
		 * WAIT_FRAME means wait n frame for exact frame update locking.
...
/*
	if (vdd->self_disp.sd_info.en | vdd->self_disp.sa_info.en) {
		LCD_INFO(vdd, "AOD Analog(Digital) Clock is enabled! - 0x00\n");
		pcmds->cmds[0].ss_txbuf[1] = 0x00;
	} else {
		LCD_INFO(vdd, "AOD Analog(Digital) Clock is disabled! - 0x01\n");
			 ...
			 size_t total_count, loff_t *ppos)
{
	...
	ret = ss_mafpc_update_enable_cmds_XA2(vdd);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_update_enable_cmds_XA2\n");
		return -EINVAL;
	}
	ret = ss_mafpc_make_img_mass_cmds_XA2(vdd, vdd->mafpc.img_buf, vdd->mafpc.img_size, TX_MAFPC_IMAGE);
	if (unlikely(ret < 0)) {
		LCD_ERR(vdd, "failed to ss_mafpc_make_img_mass_cmds_XA2\n");
		return -EINVAL;
	}
...
int ss_mafpc_init_XA2(struct samsung_display_driver_data *vdd)
{
	...
	vdd->mafpc.make_img_mass_cmds = ss_mafpc_make_img_mass_cmds_XA2;
	vdd->mafpc.make_img_cmds = ss_mafpc_make_img_cmds_XA2;
	vdd->mafpc.img_write = ss_mafpc_img_write;
	vdd->mafpc.debug = ss_mafpc_debug;
	vdd->panel_func.br_func[BR_FUNC_MAFPC_SCALE] = ss_mafpc_brightness_scale_XA2;

	vdd->mafpc.brightness_scale_table_size = sizeof(brightness_scale_table);

	vdd->mafpc.enable_cmd_size = MAFPC_ENABLE_COMMAND_LEN_XA2;
	vdd->mafpc.enable_cmd_buf = kzalloc(MAFPC_ENABLE_COMMAND_LEN_XA2, GFP_KERNEL);
